...
- The optimizer's default numpy GP + q-ParEGO path is useful as an offline baseline, but it is not enough to call the project production-ready. The BoTorch qNEHVI and qLogNEHVI paths now run, and the small synthetic benchmark in `docs/OPTIMIZATION_BENCHMARK.md` favored the BoTorch hypervolume backends over q-ParEGO on mean normalized final HV. BoTorch still emits warnings for the legacy qNEHVI test path; prefer `backend="botorch-log"` for production-style runs when the installed BoTorch version provides it.
